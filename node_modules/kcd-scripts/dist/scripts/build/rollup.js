...
function go() {
  ...
  if (hasTypescript && !args.includes('--no-ts-defs')) {
    ...
    const typeDefFiles = rollupInputs.map(input => {
      return input.replace(path.join(process.cwd(), 'src'), 'dist').replace(/\.(t|j)sx?$/, '.d.ts');
    });
    for (const format of formats) {
      ...
      const outputs = isCodesplitting ? glob.sync(toPOSIX(fromRoot(path.posix.join(dirpath, format, '*.js')))) : [fromRoot(path.join(dirpath, filename))];
      for (const output of outputs) {
        const {
          name,
          dir
        } = path.parse(output);
        const typeDef = isCodesplitting ? typeDefFiles.find(f => path.basename(f) === `${name}.d.ts`) : 'dist/index.d.ts';
        const relativePath = path.join(path.relative(dir, process.cwd()), typeDef).replace(/\.d\.ts$/, '');
        // make a .d.ts file for every generated file that re-exports index.d.ts
        fs.writeFileSync(path.join(dir, `${name}.d.ts`), `export * from "${relativePath}";\n`);
      }
    }

    // because typescript generates type defs for ignored files, we need to
    // remove the ignored files
    const ignoredFiles = [...glob.sync(toPOSIX(fromRoot('dist', '**/__tests__/**'))), ...glob.sync(toPOSIX(fromRoot('dist', '**/__mocks__/**')))];
