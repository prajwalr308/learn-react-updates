...
if (script && script !== '--help' && script !== 'help') {
  ...
  const scriptsAvailable = glob.sync(toPOSIX(path.join(__dirname, 'scripts', '*')));
...
function spawnScript() {
  ...
  const scriptIndex = args.findIndex(x => ['build', 'format', 'lint', 'pre-commit', 'test', 'validate', 'typecheck'].includes(x));
  ...
  const result = spawn.sync(executor, nodeArgs.concat(scriptPath).concat(args.slice(scriptIndex + 1)), {
    stdio: 'inherit',
    env: getEnv()
  });
  if (result.signal) {
